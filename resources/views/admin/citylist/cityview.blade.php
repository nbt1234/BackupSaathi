  ...
  <div class="row">
    <div class="col-12">
      <div class="card">  
         ...
         <a href="{{url('admin/add-city-view/')}}"><button type="button" class="btn btn-default" data-toggle="modal" >Add City</button></a>
          </div>             
        <div class="card-body">
                ...
                <thead>
                  ...
                  <tbody> 
                        ...
                        @foreach($cities as $city)
                            <tr>
                            <td>{{$count++}}</td>
                                <td><img src="{{URL::asset('/admin-assets/img/city/')}}/{{$city['image']}}" alt="" height="60" width="60"></td>
                                <td>{{$city['city_name']}}</td>  
                                <td>
                                @if($city['status'] == 1)
                                  <a class="text-success" onclick="confirm_box_status(0,{{$city['id']}},'admin/city-status',this,'status')"><strong>Active</strong></a>
                                  
                                  @else
                                   <a class="text-danger" onclick="confirm_box_status(1,{{$city['id']}},'admin/city-status',this,'status')"><strong>Inactive</strong></a>
                                  
                                  @endif
                                </td> 
                                <td>
                                <a class="btn btn-primary" href="{{url('admin/edit-city/'.$city['id'])}}"><i class="fa fa-edit"></i></a>
                                  <a class="btn btn-danger show_confirm" data-id = "{{$city['id']}}" ><i class="fa fa-trash"></i></a>
